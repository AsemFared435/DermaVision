...
const Result = () => {
      ...
      if (response.success) {
        ...
        setLoadedAnalysisResult({
          ...
          secondaryDiseases: (data.all_predictions || []).slice(1).map(pred => ({
            ...
            probability: confidenceToPercent(pred.probability),
          })),
  ...
  const isUncertain = Boolean(analysisResult.isUncertain || analysisResult.resultStatus === 'uncertain');
  const uncertaintyReasons = getUncertaintyReasons(analysisResult.uncertaintyReasons || [], analysisResult.probability);
  const uncertaintyMessage = uncertaintyReasons.length > 0
    ? getUncertaintyMessage(uncertaintyReasons, isArabic)
    : (analysisResult.userMessage || getUncertaintyMessage(uncertaintyReasons, isArabic));
  ...
  const handlePrintPDF = () => {
    ...
    const safeUncertaintyReasons = uncertaintyReasons
      .map((reason) => getUncertaintyReasonLabel(reason, isArabic))
      .map(escapeHtml);
    const safeLabels = {
      ...
      uncertainResult: escapeHtml(isArabic ? 'نتيجة غير مؤكدة' : 'Uncertain Result'),
      uncertaintyMessage: escapeHtml(uncertaintyMessage),
    };
    ...
    .uncertainty { background: #fffbeb; border: 2px solid #fde68a; border-radius: 8px; padding: 12px; margin-bottom: 12px; color: #92400e; }
    .uncertainty-title { font-weight: 800; margin-bottom: 6px; }
    .uncertainty ul { margin-top: 6px; padding-${isArabic ? 'right' : 'left'}: 18px; }
    ...
    ${isUncertain ? `<div class="uncertainty"><div class="uncertainty-title">⚠️ ${safeLabels.uncertainResult}</div><p>${safeLabels.uncertaintyMessage}</p>${safeUncertaintyReasons.length > 0 ? `<ul>${safeUncertaintyReasons.map(reason => `<li>${reason}</li>`).join('')}</ul>` : ''}</div>` : ''}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <FadeSection delay={0}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden mb-6">
            ...
            <div className="p-8">
              {isUncertain && (
                <div className="mb-6 rounded-2xl border-2 border-yellow-300 bg-yellow-50 p-5 shadow-sm dark:border-yellow-700 dark:bg-yellow-900/20">
                  <div className="flex items-start gap-4">
                    <FaExclamationTriangle className="mt-1 flex-shrink-0 text-2xl text-yellow-600 dark:text-yellow-400" />
                    <div>
                      <div className="mb-2 inline-flex rounded-full bg-yellow-200 px-3 py-1 text-xs font-black uppercase tracking-wide text-yellow-900 dark:bg-yellow-800/60 dark:text-yellow-100">
                      ...
                      {uncertaintyReasons.length > 0 && (
                        <ul className="mt-3 list-disc space-y-1 ps-5 text-sm text-yellow-800 dark:text-yellow-200">
                          {uncertaintyReasons.map((reason) => (
                            <li key={reason}>{getUncertaintyReasonLabel(reason, isArabic)}</li>
                          ))}
                        </ul>
                      )}
                      <p className="mt-3 text-xs leading-relaxed text-yellow-700 dark:text-yellow-300">
                        {isArabic
                          ? 'هذا لا يعني أن النظام يكتشف كل الأمراض غير المدعومة أو الصور غير الجلدية؛ هو فقط يقلل النتائج المفرطة الثقة عند انخفاض الثقة أو الجودة أو تقارب التوقعات.'
                          : 'This does not mean the system can detect every unsupported disease or non-skin image; it only reduces overconfident results when confidence, image quality, or prediction margin is weak.'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {isUncertain && (
                        <span className="rounded-full bg-yellow-100 px-3 py-1 text-xs font-black text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300">
