...
السيرفر يعمل محلياً على: http://127.0.0.1:8000

إذا كان port 8000 مستخدماً على Windows، افحص العملية:

```powershell
netstat -ano | findstr :8000
```

يمكنك إيقاف العملية أو استخدام port 8001 مؤقتاً، لكن port 8000 هو الإعداد المحلي الافتراضي.
...
الطلب: `multipart/form-data`، حقل `file` (JPG, JPEG, PNG، حد أقصى 10MB)
...
curl -X POST "http://127.0.0.1:8000/api/v1/diagnosis" \
  -H "Authorization: Bearer $TOKEN" \
  -F "file=@test_image.jpg"

# السجل
curl -H "Authorization: Bearer $TOKEN" \
  "http://127.0.0.1:8000/api/v1/history?limit=10"
```
