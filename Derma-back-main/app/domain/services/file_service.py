...
class FileService:
    ...
    async def save_upload(
        ...
        unique_name = f"{session_id or uuid4().hex}{file_ext}"
        ...
        try:
            ...
            logger.info("File saved for session: %s", session_id or "generated")
            return file_path
    ...
    async def _validate_image_content(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.warning("Invalid uploaded image content: %s", e)
            raise ValidationError(
                "Invalid image file",
                details={"reason": "File is not a valid image"}
            )
